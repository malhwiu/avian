...
pub struct MoveAndSlideOutput {
    ...
    ///
    /// It is useful to store this value or apply it to [`Velocity::linear`] and use it as the input velocity
    /// for the next frame's call to [`MoveAndSlide::move_and_slide`].
    ///
    /// Note that if you apply this to [`Velocity::linear`], it is recommended to use [`CustomPositionIntegration`].
